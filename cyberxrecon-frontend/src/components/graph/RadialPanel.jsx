...
export default function RadialPanel({ data }) {
  ...
  // Convert static data on target change to local React State to allow dynamic spawning
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      if (graphRef.current) graphRef.current.zoomToFit(400, 36);
    }, 150);
  ...
  // Pre-Render Canvas Frame (Background Sonar rings and sweep needle)
  const drawRadarGrid = (ctx, globalScale) => {
    ...
    ctx.beginPath();
    // Speed adjusted dynamically by state
    ...
    ctx.lineWidth = (scanningNodeId ? 2.5 : 1.5) / globalScale;
    ctx.stroke();

    ctx.restore();
  };

  // Node Canvas Object painter
  ...
  // Right-click context menu (Maltego transforms inside radar)
  const renderContextMenu = () => {
    if (!contextMenu) return null;

    const node = contextMenu.node;
    if (node.group === 1) return null; // Ignore center target
    ...
    <div className="relative bg-[#020205]/60 backdrop-blur-md border border-white/10 rounded-2xl overflow-hidden flex flex-col min-h-0 select-none">
      ...
      <div ref={containerRef} className="flex-1 relative min-h-0">
        <ForceGraph2D
          ...
          cooldownTicks={Infinity} // Prevents the canvas from freezing
          cooldownTime={Infinity}  // Keeps the animation loop alive indefinitely
        ...
        {/* 3. Floating HUD Target Information Card Overlay */}
        {selectedNode && selectedNode.group !== 1 && (
          <div className="absolute bottom-3 left-3 right-3 bg-[#05050a]/95 border border-purple-500/20 rounded-xl p-3 shadow-2xl shadow-black/95 animate-fade-in font-mono z-20">
            ...
            {/* DEEP SCAN HUD BUTTON (Active Spawning Trigger) */}
            <div className="mt-2.5 pt-2 border-t border-white/5">
              ...
              >
                {scanningNodeId ? '🛰️ RUNNING SIGNAL EXTRAPOLATION...' : '⚡ Run Deep Signal Scan'}
              </button>
            </div>
          </div>
        )}
